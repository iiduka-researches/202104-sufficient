...
class ConjugateGradient(Solver):
    ...
    def __init__(self, beta_type=BetaTypes.DY, orth_value=np.inf, linesearch=None, *args, **kwargs):
        """
            ...
            if abs(orth_grads) >= self._orth_value:
                ...
                if self._beta_type == BetaTypes.FR:
                    ...
                    try:
                        ...
                        beta = 1
                elif self._beta_type == BetaTypes.HZ:  # HZ
                    diff = newgrad - oldgrad
                    ...
                    diff = newgrad - oldgrad
                    ip_diff = man.inner(newx, Pnewgrad, diff)
                    beta_PR = ip_diff / gradPgrad
                    beta = max(0, min(beta_FR, beta_PR))
                else:
...
class LineSearchWolfe:
    def __init__(self, c1: float=1e-4, c2: float=0.9, strong=True):
        self.c1 = c1
        self.c2 = c2
        self.strong = strong
